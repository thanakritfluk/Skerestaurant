...
public class SkeRestaurant {
        ...
        public static void editOrder(String choice){
            if(choice.equalsIgnoreCase("e")) {
                int numChange=getQuatity("***** Edit order *****\nEnter number of menu: ");

                RestaurantManager.menuQuantity[numChange-1]=getQuatity("Change to: ");
            ...
            while (0 == 0) {
                ...
                for(int i=0;i<RestaurantManager.menuPrice.size();i++) {
                    if (choice.equalsIgnoreCase(Integer.toString(RestaurantManager.menuNum[i]))) {
                        RestaurantManager.menuQuantity[i]+=getQuatity("Enter Quantity: ");
                        sc.nextLine();
                    }
